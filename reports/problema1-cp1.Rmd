...
Análise exploratória de dados do [IMDB](https://www.imdb.com/)  sobre seriados de TV e Streaming. Os dados originais e as variáveis vêm [deste repositorio](https://github.com/nazareno/imdb-series). Lá tem a explicação de como os dados foram gerados e do significado de cada variável.
...
O conjunto de dados coletados contém informações sobre algumas séries populares na atualidade.
Na análise desse notebook foram escolhidas 3 séries bastante populares atualmente, são elas:
...
Analisando os dados escolhidos observa-se que os dados não possuem valores estranhos e não possuem um número de variação considerável e desse modo podemos analisar e utilizar o conjunto de dados sem necessariamente utilizar uma filtragem nos dados.

## Primeira pergunta 
Analisaremos o quão bem avaliada as séres no [IMDB](https://www.imdb.com/)  e desse modo tentar a chegar a uma aproximação de qual serie diante do conjunto escolhido seria a mais bem avaliada de acordo com os votos dos usuários.
...
Podemos observar neste gráfico que concentração de notas de House of Cards em sua maioria superior a 8,5 algo que não observamos nas outras séries avaliadas.
As demais séries verificamos uma diferença pequena em relação às médias.

Notamos que The Blacklist possui um número um pouco superior de episódios, e verificamos também que existem dados discrepantes com nota igual a 7.
...
De acordo com gráfico acima observamos que a série The Blacklist possui uma variação de pontos atribuídos como podemos observar que na primeira temporada a pontuação atribuída se concentra em algo entre 8,0 e 9,0 e ao decorrer das próximas temporadas observamos que os que pontos que significa os voto atribuído vão diminuindo e na quarta temporada os pontos se dividem na metade ficando uma parte entre 7,0 e 8,0 desse modo verificamos que ao longo das temporadas ocorreu uma mudança considerável.
Em relação à série Vikings enxergamos que existe uma diferença da primeira em relação à segunda temporada onde a concentração de pontos está entre 8,0 e 9,0 e mantendo nas temporadas subsequentes.
...
No, mas verificamos que as três séries se mantêm no intervalo de 8,0 e 9,0 na avaliação atribuídas a seus episódios á partir disto verificamos que os episódios são bem avaliados em sua maioria.
